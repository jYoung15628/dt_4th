...
// 값 또는  참조가 어떻게 전달되는지? 에 대해 배워보겟습니다
...
let num =1;
let num2 = num // 값의 복사가 일어남. => num 값을 num2에 복사함
// 두 변수는 메모리에서 별도의 공간에 저장됨 => 서로 독립적인 존재

console.log('num:', num, 'num2:', num2);
console.log(num === num2); // true, 엄격한 동등 검사,  값에 의한 전달이기 때문에 값을 비교
// 새로 변수를 선얼할때 새 주소값이 할당하기 때문
...
changeValue(num); // x > 10  // num =5 가 x에 전달하면서 x라는 껍데기가 씌워진다고 생각하면 됨 그래서 
// num = x = 10 이므로 x > 10 이라는 값을 출력함 ! 
console.log('num >>>>', num); // 5 , 하지만 함수를 벗어나면 X라는 껍데기는 벗겨지기 때문에 num = 5 가 나옴

// (풀이)
// 함수로 전달 될 때, 변수의 값이 복사되어 함수의 매개변수로 전달
// 따라서 함수 내에서 매개변수의 값을 변경하더라도 원본 변수의 값은 변하지 않음
// why? 함수 내에서 사용되는 변수는 그 함수 스코프 내에서만 유효한 "지역변수"이기 때문
...
console.log(obj === obj2); // true, 참조에 의한 전달이기 때문에 주소를 비교
