...
// 함수표현식 (익명함수)
// 화살표 함수 
btn2.addEventListener('click', () => { // 함수 표현식
    const div = document.createElement('div');
    div.style.backgroundColor='pink';
    div.innerHTML= 'Hi~!';
    container.append(div); // container 에 붙임
});
...
//  * this 주의사항
//  일반 함수 : 'this' 는 이벤트를 발생시킨 dOM요소를 가르킴
...
//  ex) mousedown 이벤트 발생 => 이벤트객체는 (마우스 좌표, 버튼 번호) 정보를 가짐
//  ex) keydown 이벤트 발생ㅇ => 이벤트 객체는 (키 코드값, 어떤 키가 눌렸는지에 대한 정보) 정보를 가졍모
...
input.addEventListener('keydown', function(e){
    ...
    conosole.log(e.key); // input에 입력된 값
