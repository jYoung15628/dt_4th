...
// 새로운 배열이나 객체를 복사하거나 병합 하는데 사용하는 ES6 기능
//  - 더 간결하고 유연하게 데이터 처리 가능

//  배열에서의 스프레드 연산자
// #1. 배열 복사
const arr = ['a', 'b', 'c'];
const copyArr = [...arr]; // 배열을 복사
// console.log(copyArr); ['a', 'b', 'c']
console.log(arr = copyArr); // false -> why? 복사본 이므로 다른 참조
...
console.log(copyObj === obj); // false  why? 서로 다른 참조 (말 그대로 그냥 복사만)
...
console.log('whopper>>>>>>', whopper);

//////////////////////////
//  rest 파라미터
//  - 함수가 여러 개의 인자를 받을 때, 나머지 인자들을 배열로 모아주는 역할
//  - "마지막 인자"로만 사용
//  - 함수에서 여러 rest 파라미터 사용 불가. 한번만 등장 가능!
...
// Spread 문법으로 배열의 개별 요소를 인자로 펼처서 전달
Details(...details); // Name : IU, Age:30

// Rest - 함수 정의 시 (배열로 모아줌)
...
/**
 * 얕은 복사 vs 깊은 복사 (기술면접으로 많이 나오몽! 중요중요!!!)
 * - 객체나 배열과 같은 참조 타입 데이터를 처리할 때 , 데이터를 복사하느 두가지 주요 방식
 * 
...
shallowCopy[1].name = "Layla";// 중첩 객체 수정 -> 원본에 영향
console.log(arrEx[1]); // {name: "Layla"} - > 참조를 공유하므로 변경


/**
 *  2. 깊은 복사 (Deep Copy)
 *  - 복사 대상 객체의 모든 데이터를 재귀적으로 완저닣 새롭게 생성함
 * - 원본과 복사본이 완전히 독립적인상태
 ...
 *  - 복사본과 원본이 완전히 분리됨
 *  - 다만, 성능저긍로 더 많은 비용(시간, 메모리)이 들 수 있음
 * 
 *  * 깊은 복사를 만드는 방법
