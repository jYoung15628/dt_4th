...
class House {
    ...
    constructor(year, name, window) {
        ...
        // ( 코드의 가독성을 위해 속성과 매개변수의 이름을 같게 지정)
        this.year = year;
        this.name = name;
        this.window = window;
    }
...
//  #5. 인스턴스 : 클래스로부터 생성된 실제 객체 - new 키워드를 사용해서 생성
//  클래스 - 설계도
//  인스턴스 - 설계도로 만들어진 "실제 집"
...
house1.getAge(); // 롯데는 건축한지 NaN년 되었어요
...
//  - 'extends' 키워드르 사용하여 상속 구현
class Apartment extends House {
    constructor(year, name, window, floor, windowMaker) {
        // #6. super() : 부모클래스의 생성자를 호출할 때 사용
    ...
    //  - 주로, 부모 클래스에서 재공한 기본 동장을 "자식 클래스의 특성에 맞게 수정" 할 때 사용

    // * 규칙
    //  - 메서드 이 름, 매개변수 리스트가 부모 클래스와 "동일" 해야함
    getWindow() {
        return `${this.window} 아파트의 ${this.window}개의 창문은 
...
console.log(apt1);
console.log(apt1.getAptInfo()); // 2024년에 지어진 레미안 아파트의 총 층수는 15이며, 창문의 개수는 3 이다
console.log(apt1.getWindow());
//  레미안 아파트의 3개의 창문은 3M 회사에서 생산되었습니다
...
const [first, second, ...remainig] = numbers;
...
// Shape 인스턴스가 아닌 Rectangle 인스턴스를 생성
let rec2 = new Rectangle(3, 4);
console.log(rec2.getDia()); // 5
