...
export default function LifeCycleFuncChild({number}) {
    ...
    useEffect(()=>{
        console.log('컴포넌트 마운트! 🍇') // 효과함수 : 마운트와 업데이트시 실행할 코드 작성
        return()=>{
            console.log('컴포넌트 언마운트! 🍎'); // 정리 함수 : 언마운트 또는 다음 useEffect 실행 전 호출
        };
    },[]); // 의존성 배열 : 배열에 지정된 값이 변경 될 때 효과 함수가 실행 
    ...
    // 의존성 배열이 [] 빈 배열이므로 #1, #2 는 렌더링시 "한번만" 실행
    // 의존성 배열을 생략하는 의미는 "매 렌더링 마다 실행" 하겠다는 뜻 (상태 변경마다)
    ...
    useEffect(()=>{
        ...
        //  - useEffect 가 실행 될 때, 이전 효과가 남아 있으면 중복된 효과가 발생할 수 있기 때문에
        //  - 이전 효과를 먼저 제거 한 후 새로운 효과 적용을 하기 위함
    }, [input]);
    // input이 배열이 아니더라도 의존성 배열에 추가할 수 있다
    // 문자열이든 숫자든 상관없이 이 값이 변경될 때마다 useEffet가 실행된다

    // #5. 여러개의 useEffect
    // - 보다시피 하나의 컴포넌트 안에 *여러개의 useEffect* 사용가능
    // - React는 가각의 useEffect를 독립적으로 관리함

    return (
        <div>
            자식 컴포넌트 <div>현재 Number 값은 {number} 입니다</div>
            <input 
                type="text"
                value={input}
